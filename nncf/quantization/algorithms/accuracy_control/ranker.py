...
class Ranker:
    ...
    def rank_groups_of_quantizers(
            ...
            approximate_values_for_each_item: Union[List[float], List[Output], None]) -> List[GroupToRank]:
        """
        ...
        """
        # If the passed parameter `reference_values_for_each_item` is not `None`
        # we should use it instead of `self._reference_values_for_each_item`.
        if reference_values_for_each_item is None:
            if self._reference_values_for_each_item is None:
                ...
                with timer():
                    self._reference_values_for_each_item = self._evaluator.collect_values_for_each_item(initial_model,
                                                                                                        self._dataset)
            reference_values_for_each_item = self._reference_values_for_each_item
        ...
        if approximate_values_for_each_item is None:
            ...
            with timer():
                approximate_values_for_each_item = self._evaluator.collect_values_for_each_item(quantized_model,
                                                                                                self._dataset)
    ...
    def _calculate_ranking_score(self, modified_model: TModel, ranking_subset_indices: List[int]) -> float:
        ...
        if self._evaluator.is_metric_mode():
            ...
            ranking_score = self._evaluator.validate(modified_model, self._dataset, ranking_subset_indices)
        else:
            # Calculate ranking score based based on differences in logits
            approximate_outputs: List[Output] = self._evaluator.collect_values_for_each_item(modified_model,
                                                                                            self._dataset,
