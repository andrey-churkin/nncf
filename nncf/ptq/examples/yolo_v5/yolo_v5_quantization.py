...
ROOT = Path(os.path.relpath(FILE.parent, Path.cwd()))  # Relative path to the `yolo_v5` directory
...
def run_example():
    ...
    if not Path(args.example_dir).exists():
    ...
    data_source = create_val_dataset(args.data)
    ...
    ir_qmodel_xml = str(args.example_dir.joinpath(f'{model_name}.xml'))
    ir_qmodel_bin = str(args.example_dir.joinpath(f'{model_name}.bin'))
    ...
    metrics = validate(data=args.data,
                       weights=ir_model_xml,
                       batch_size=1,
                       workers=1,
                       plots=False,
                       device='cpu',
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description='Quantization of the YOLOv5 model',
    ...
    parser.add_argument(
        ...
        metavar='DIR',
        nargs='?',
        default=ROOT.joinpath('yolov5m_quantization'),
        help='path to a directory where the quantized model will be saved'
    )
    parser.add_argument(
        '--data',
        metavar='YAML FILE',
        nargs='?',
        default=ROOT.joinpath('yolov5', 'data', 'coco.yaml'),
...
def torch_to_openvino_model(example_dir: str) -> Tuple[str, str]:
    ...
    ir_model_xml = Path(example_dir).joinpath(f'{model_name}.xml')
    ir_model_bin = Path(example_dir).joinpath(f'{model_name}.bin')
    return str(ir_model_xml), str(ir_model_bin)


def create_val_dataset(yaml_path: str) -> torch.utils.data.Dataset:
    """
    Creates COCO 2017 validation dataset. The method downloads COCO 2017
    dataset if it does not exist.

    :param yaml_path: Config of the dataset.
